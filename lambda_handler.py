...
def notify_sms(settings, dates, location_name):
    try:
        ...
        for avail_apt in dates:
            body = "New Global Entry appointment available at %s on %s" % (
                location_name,
                avail_apt,
            )
...
def check_appointments(settings):
    try:
        response = requests.get(
            GOES_URL_FORMAT.format(settings["enrollment_location_id"])
        )
        data = response.json()
        ...
        current_apt = datetime.strptime(
            settings["current_interview_date_str"], "%Y-%m-%d"
        )
        dates = []
        for slot in data:
            if slot["active"]:
                appointment_date = datetime.strptime(
                    slot["startTimestamp"], "%Y-%m-%dT%H:%M"
                )
        ...
        if dates:
            location_name = settings.get(
                "enrollment_location_name", settings["enrollment_location_id"]
            )
            notify_sms(settings, dates, location_name)
        else:
            logging.info(
                "No new appointments found that are earlier than the current one."
            )
